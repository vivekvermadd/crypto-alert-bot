...
cursor = conn.cursor()
cursor.execute('''CREATE TABLE IF NOT EXISTS alerts (user_id INTEGER, alert_id TEXT PRIMARY KEY, data TEXT)''')
conn.commit()

alerts = defaultdict(dict)  # user_id: {alert_id: {'exchange':str, 'symbol':str, 'limit':float, 'direction':str}}
tasks = defaultdict(list)  # user_id: [task coros]
...
async def save_alert(user_id, alert_id, alert):
    cursor.execute('INSERT OR REPLACE INTO alerts VALUES (?, ?, ?)', (user_id, alert_id, json.dumps(alert)))
    conn.commit()

def get_exchange_obj(ex_id):
    return {'binance': ccxt.binance(), 'bybit': ccxt.bybit(), 'htx': ccxt.htx(),
            'kucoin': ccxt.kucoin(), 'gate': ccxt.gate(), 'bitmart': ccxt.bitmart()}[ex_id]
...
async def set_alert_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("Select Exchange:", reply_markup=kb)
...
async def set_limit(message: types.Message, state: FSMContext):
    ...
    await message.reply("Direction:", reply_markup=kb)
...
async def set_dir(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📋 List", callback_data="list_alerts")]])
    await callback.message.edit_text(f"✅ Alert set!\n{data['exchange'].upper()} {data['symbol']} {direction} {data['limit']}", reply_markup=kb)
    await state.clear()
    await callback.answer()
...
async def list_alerts(callback: CallbackQuery):
    ...
    if not alerts[user_id]:
        ...
        return
    text = "Alerts:\n" + "\n".join([f"{aid}: {a['exchange'].upper()} {a['symbol']} {a['direction']} {a['limit']}" for aid, a in alerts[user_id].items()])
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🗑️ Del All", callback_data="del_all")] ] + 
        [[InlineKeyboardButton(text=f"🗑️ {aid[:20]}...", callback_data=f"del_{aid}")] for aid in alerts[user_id]])
...
async def del_alert(callback: CallbackQuery):
    ...
    await list_alerts(callback)  # Refresh
    ...
    async def monitor():
        ...
        try:
            while alert_id in alerts[user_id]:
                ...
                if (direction == 'above' and price >= alert['limit']) or (direction == 'below' and price <= alert['limit']):
                    try:
                        await bot.send_message(user_id, f"🚨 ALERT: {alert['exchange'].upper()} {alert['symbol']} {price:.4f} ({direction} {alert['limit']})")
                    except:
                        pass  # Rate limit skip
                    await stop_ws_monitor(user_id, alert_id)  # One-time
                    break
...
async def main():
    await load_alerts()
    # Restart WS for existing
    for uid, ualerts in alerts.items():
        for aid, alert in ualerts.items():
            await start_ws_monitor(uid, aid, alert)
